// src/utils/platformRouter.js
// ARIA Platform Router — shared utility
// Every controller that needs platform context uses this
// Ensures YouTube users never hit Instagram logic and vice versa
'use strict';

/**
 * Get platform-aware context for any ARIA service call
 * Use this in every controller instead of raw user.primaryPlatform
 *
 * @param {object} user - req.user from Firebase auth middleware
 * @returns {object} { platform, handle, isInstagram, isYouTube, niche, archetype, followerRange }
 */
const getPlatformContext = (user) => {
  ...
  return {
    ...
    followerRange: user.followerRange || '1K–10K',
 ...
 */
const buildPlatformPromptContext = (user) => {
 ...
 */
const getPlatformTimingWindows = (archetype, platform) => {
...
module.exports = { getPlatformContext, buildPlatformPromptContext, getPlatformTimingWindows };
