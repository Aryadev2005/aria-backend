...
// ARIA Studio — pre-production intelligence
// Script structure, BGM matching, shot list, editing guidance
'use strict';
...
// ─── BGM Matcher ──────────────────────────────────────────────────────────────
const matchBGM = async ({ idea, mood, niche, platform, archetype, duration }) => {
  // Try live songs from DB first
  let liveSongs = [];
  try {
    const sql    = getDB();
    liveSongs    = await sql`
      SELECT title, artist, chart_position, lifecycle, posting_signal
      FROM live_songs
  ...
  const liveContext = liveSongs.length > 0
    ? `CURRENTLY TRENDING SONGS IN INDIA:\n` +
      liveSongs.slice(0, 8).map(s =>
        `• "${s.title}" by ${s.artist} — position #${s.chart_position}, ${s.lifecycle}`
      ).join('\n')
    : '';
...
// ─── Shot List Generator ──────────────────────────────────────────────────────
const generateShotList = async ({ idea, format, niche, archetype, sections }) => {
...
// ─── Editing Help ─────────────────────────────────────────────────────────────
const getEditingHelp = async ({ problem, tool, niche, archetype }) => {
  const EDITING_KB = {
    CapCut:   'CapCut mobile app — has auto-captions, templates, effects, speed control, keyframes',
...
const saveStudioSession = async (userId, sessionData) => {
  try {
    const sql = getDB();
    const [session] = await sql`
      INSERT INTO studio_sessions (
        user_id, idea, platform, niche,
        script_structure, bgm_suggestions, shot_list
      ) VALUES (
        ${userId},
        ${sessionData.idea},
        ${sessionData.platform},
        ${sessionData.niche},
        ${JSON.stringify(sessionData.scriptStructure || {})},
        ${JSON.stringify(sessionData.bgmSuggestions  || {})},
        ${JSON.stringify(sessionData.shotList         || {})}
      )
      RETURNING id
    `;
    return session.id;
