...
const INDIAN_FESTIVALS: Record<string, Record<number, string>> = {
...
export interface CreatorAnalyseParams {
  niche: string;
  platform: string;
  followerRange: string;
...
const ariaService = {
  ...
  // ── 1. CONTENT CALENDAR ───────────────────────────────────────────────────
  async generateCalendar({ niche, platform, followerRange, month, year }: CalendarParams) {
  ...
  async getTrendingIdeas({ niche, platform, followerRange, archetype }: IdeaParams) {
  ...
  async generateContent({ trendTitle, niche, platform, followerRange, archetype, contentFormat }: ContentGenParams) {
  ...
  // ── 4. TRENDING SONGS ─────────────────────────────────────────────────────
  async getTrendingSongs({ niche, platform, followerRange }: Partial<IdeaParams>) {
  ...
  async generateRateCard({ niche, platform, followerRange, engagementRate, topPosts }: RateCardParams) {
  ...
  // ── 6. CAPTION ANALYSIS ───────────────────────────────────────────────────
  async analyzeCaption({ caption, niche, platform }: AnalysisParams) {
  ...
  async analyzeBio({ bio, niche, platform }: AnalysisParams) {
  ...
  generateCalendarFallback({ niche, platform, month, year }: CalendarParams) {
  ...
  _monthIndex(month: string) {
    return new Date(`${month} 1, 2024`).getMonth();
  },

  async analyseCreator({ niche, platform, followerRange, engagementRate, postsPerMonth }: CreatorAnalyseParams) {
