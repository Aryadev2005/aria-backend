// src/services/youtubeTrending.service.js
// Real YouTube Trending data for India using YouTube Data API v3
// Uses existing YOUTUBE_API_KEY from .env
// Called by trend.worker.js every 6 hours
 ...
 * Returns 0-100 score
 */
const calculateVelocity = (viewCount, likeCount, commentCount) => {
...
const fetchYouTubeTrending = async () => {
  ...
  try {
    ...
    const trends = videos.map((video, idx) => {
 ...
 */
const searchYouTubeByNiche = async (niche, maxResults = 10) => {
  const apiKey = process.env.YOUTUBE_API_KEY;
...
module.exports = { fetchYouTubeTrending, searchYouTubeByNiche };
