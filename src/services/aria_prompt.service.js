'use strict'
// aria_prompt.service.js
// Builds the dynamic ARIA system prompt for every chat session.
// Injects: archetype, analytics, memory, screen context, session idea/script.
// This replaces the static ARIA_SYSTEM string in ariaService.js.

const { buildMemoryBlock } = require('./aria_memory.service')

// ── Archetype hook strategies ────────────────────────────────────────────────
const ARCHETYPE_HOOKS = {
  TRENDSETTER: {
    hookStyle:   'aspiration and aesthetic — "look expensive for less", "quiet luxury for ₹500"',
    contentBias: 'Reels with trending audio, outfit reveals, haul content',
    toneNote:    'Inspiring, aspirational, visually descriptive',
  },
  EDUCATOR: {
    hookStyle:   'stat-led authority — "5 things nobody tells you about X", "the truth about Y"',
    contentBias: 'Carousels, explainer Reels, myth-busting content',
    toneNote:    'Confident, data-backed, clear and structured',
  },
  ENTERTAINER: {
    hookStyle:   'shock and relatable pain — "POV: you just...", "tell me you\'re Indian without telling me"',
    contentBias: 'Short Reels, trending audio, reaction content, memes',
    toneNote:    'Fast, punchy, high energy, desi humor',
  },
  STORYTELLER: {
    hookStyle:   'narrative arcs — "I almost quit last year...", "my honest review after 6 months"',
    contentBias: 'Vlogs, long-form YouTube, travel content, day-in-my-life',
    toneNote:    'Warm, personal, emotionally engaging',
  },
  CONNECTOR: {
    hookStyle:   'vulnerability and authenticity — "nobody talks about this", "I never share this but..."',
    contentBias: 'Stories, Q&As, community polls, candid posts',
    toneNote:    'Honest, warm, community-first',
  },
  EXPERT: {
    hookStyle:   'deep authority — "the real truth about X", "why 90% of people get this wrong"',
  ...
  HUSTLER: {
    hookStyle:   'numbers and proof — "how I made ₹X in 30 days", "the exact strategy that got me to 10K"',
...
const SCREEN_CONTEXT = {
  studio: 'The user just came from the Studio screen. They are actively working on a script or piece of content. Lead with content-specific advice.',
  discover: 'The user just came from the Discover screen. They are in trend exploration mode. Lead with timely, actionable trend advice.',
  launch: 'The user just came from the Launch screen. They are about to post or scheduling content. Lead with timing, caption, and hashtag optimization.',
  profile: 'The user just came from their Profile screen. They are reviewing their analytics. Lead with growth insights and performance patterns.',
  direct: 'The user opened Brain directly. Ask what they are working on before giving advice.',
}

// ── Emotional calibration based on health score ──────────────────────────────
const getEmotionalRegister = (healthScore, recentGrowth) => {
...
const buildARIASystemPrompt = ({
  ...
  const archetypeData = ARCHETYPE_HOOKS[archetype] || ARCHETYPE_HOOKS.TRENDSETTER
  const archetypeLabel = user?.archetype_label || archetype
  const growthStage = user?.growth_stage || 'GROWTH'
  const toneProfile = user?.tone_profile || 'casual'
  ...
  const primaryPlatform = user?.primary_platform || 'instagram'
  const niches = Array.isArray(user?.niches) ? user.niches.join(', ') : (user?.niches || 'general')

  // Session context (idea/script/platform passed from Flutter)
  const hasSessionContext = sessionContext.idea || sessionContext.script || sessionContext.platform
...
${sessionContext.trendTitle ? `- Trend they picked: "${sessionContext.trendTitle}"` : ''}` : ''
  ...
  const followUpBlock = pendingSuggestions.length > 0 ? `
...
module.exports = { buildARIASystemPrompt }
