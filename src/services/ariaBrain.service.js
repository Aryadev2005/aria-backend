...
// We wrap in try/catch so the server starts even if files are missing
let getLiveTrendsForNiche = async () => []
let searchYouTubeByNiche  = async () => []
let getUpcomingFestivals  = () => []
let getPlatformContext    = (user) => ({
  platform:     user.primary_platform || user.primaryPlatform || 'instagram',
  niche:        Array.isArray(user.niches) ? user.niches[0] : (user.niches || 'lifestyle'),
  followerRange: user.follower_range || user.followerRange || '10K-50K',
})

try {
  const radar = require('./radar.service')
  if (radar.getLiveTrendsForNiche) getLiveTrendsForNiche = radar.getLiveTrendsForNiche
  if (radar.getUpcomingFestivals)  getUpcomingFestivals  = radar.getUpcomingFestivals
} catch (e) {
  logger.warn('radar.service.js not found — live trends will be empty in Brain context')
}

try {
  const yt = require('./youtubeTrending.service')
  if (yt.searchYouTubeByNiche) searchYouTubeByNiche = yt.searchYouTubeByNiche
} catch (e) {
  logger.warn('youtubeTrending.service.js not found — YouTube context will be empty')
}

try {
  const router = require('../utils/platformRouter')
  if (router.getPlatformContext) getPlatformContext = router.getPlatformContext
} catch (e) {
  logger.warn('platformRouter.js not found — using default platform context')
}
...
const getMemory = async (userId) => {
  ...
  try {
    ...
    const mem = {}
    for (const r of rows) {
      mem[r.key] = { value: r.value, type: r.memory_type, confidence: r.confidence }
    }
    await cache.set(cacheKey, mem, 180) // 3 min cache
    return mem
  } catch (err) {
    logger.warn({ err, userId }, 'Memory fetch failed — proceeding without memory')
    return {}
  }
}
...
const learnFromConversation = async (userId, userMessage) => {
  ...
  const patterns = [
    ...
    // Content language
    { match: ['in hindi', 'speak hindi', 'hindi content'], key: 'content_language', value: 'Hindi',         type: 'style' },
    { match: ['hinglish'],                                key: 'content_language',  value: 'Hinglish',      type: 'style' },
    { match: ['in english', 'english only'],              key: 'content_language',  value: 'English',       type: 'style' },
    // Goals
    ...
    { match: ['1 million', '1m followers'],               key: 'follower_target',   value: '1M',            type: 'goal' },
    { match: ['brand deal', 'brand deals', 'sponsorship'], key: 'monetisation_goal', value: 'brand_deals',  type: 'goal' },
    { match: ['solo', 'alone', 'by myself'],              key: 'collab_preference', value: 'solo',          type: 'preference' },
    { match: ['collab', 'collaboration', 'with someone'], key: 'collab_preference', value: 'collab',        type: 'preference' },
  ]
...
const buildLiveContext = async (user, memory) => {
  ...
  try {
    ...
    logger.warn({ err: err.message, niche }, 'Live context timed out — proceeding with partial context')
...
Followers: ${user.follower_range || user.followerRange || 'unknown'} | Engagement: ${user.engagement_rate || user.engagementRate || 'unknown'}%
${memoryLines ? `\n══ WHAT ARIA REMEMBERS ══\n${memoryLines}` : ''}
${liveData    ? `\n══ LIVE INTELLIGENCE ══\n${liveData}` : ''}`
}
...
const persistSession = async (userId, sessionId, messages) => {
  ...
  try {
    const sql = getDB()
    // Keep last 50 messages — prevents unbounded growth
