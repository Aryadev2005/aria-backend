...
// A 50KB YouTube response becomes ~400 tokens. A 200KB Spotify response becomes ~300 tokens.
// This is the token efficiency layer.

// ── YouTube Video ─────────────────────────────────────────────────────────────
const normalizeYouTubeVideo = (raw) => {
...
// ── YouTube Channel ───────────────────────────────────────────────────────────
const normalizeYouTubeChannel = (raw) => {
  ...
  return {
    ...
    avgViewsPerVideo: s.videoCount > 0
      ? Math.round(parseInt(s.viewCount) / parseInt(s.videoCount))
      : 0,
  }
}

// ── YouTube Search Results ────────────────────────────────────────────────────
const normalizeYouTubeSearch = (raw) => {
  if (!raw?.items) return { results: [] }
  return {
    totalResults: raw.pageInfo?.totalResults,
    results: raw.items.slice(0, 8).map(item => ({
...
// ── Spotify Charts ────────────────────────────────────────────────────────────
const normalizeSpotifyTracks = (rawTracks) => {
  if (!Array.isArray(rawTracks)) return { tracks: [] }
  return {
    tracks: rawTracks.slice(0, 20).map((t, i) => {
      // Handle both chartEntryData and direct track formats
      ...
      return {
        ...
        artist:   (meta.artists || []).map(a => a.name || a).join(', '),
...
const normalizeJioSaavnSongs = (rawSongs) => {
  if (!Array.isArray(rawSongs)) return { songs: [] }
  return {
    songs: rawSongs.slice(0, 15).map((s, i) => ({
      rank:     i + 1,
      title:    s.title || s.name,
      artist:   s.primaryArtists || s.artists?.map?.(a => a.name)?.join(', ') || 'Unknown',
      language: s.language,
...
// ── Google Trends ─────────────────────────────────────────────────────────────
const normalizeGoogleTrends = (rawTrends) => {
  if (!rawTrends) return { topics: [] }

  // Handle both array format and object format from different trend sources
  const items = Array.isArray(rawTrends)
    ? rawTrends
    : (rawTrends.default?.trendingSearchesDays?.[0]?.trendingSearches || [])

  return {
    topics: items.slice(0, 15).map(t => ({
      title:         t.title?.query || t.title || t,
      traffic:       t.formattedTraffic || t.search_volume || 'N/A',
      relatedTopics: (t.relatedTopics || []).slice(0, 3).map(r => r.topic?.title || r),
...
const normalizePytrends = (rawData) => {
  if (!rawData) return { keywords: [] }

  // rawData is typically { keyword: [{date, value}] }
  return {
    keywords: Object.entries(rawData).map(([keyword, series]) => {
      const values = Array.isArray(series) ? series.map(p => p.value || p) : []
...
const normalizeInstagramMedia = (rawMedia) => {
  if (!rawMedia?.data) return { posts: [] }
  return {
    posts: rawMedia.data.slice(0, 20).map(post => ({
      id:           post.id,
...
const normalizeInstagramInsights = (rawInsights) => {
  if (!rawInsights?.data) return {}
  const metrics = {}
  rawInsights.data.forEach(d => {
  ...
  return {
    trends: rows.map(r => ({
      ...
      expiresIn:   r.expires_at
        ? Math.round((new Date(r.expires_at) - Date.now()) / 3600000) + 'h'
        : null,
    })),
  }
}
