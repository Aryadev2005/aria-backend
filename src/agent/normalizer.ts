...
export const normalizeGoogleTrends = (rawTrends: any) => {
  if (!rawTrends)
    return { timeline: [], averageInterest: 0, direction: "stable" };

  try {
    // Handle interestOverTime API response format
    if (rawTrends.default && rawTrends.default.timelineData) {
      const timeline = rawTrends.default.timelineData || [];
      if (!timeline.length)
        return { timeline: [], averageInterest: 0, direction: "stable" };

      const values = timeline
        .map((t: any) => t.value?.[0] || 0)
        .filter((v: number) => v > 0);
      const averageInterest = values.length
        ? Math.round(
      ...
      const recentValues = values.slice(-3); // Last 3 data points
      const direction =
...
export const normalizeInstagramMedia = (rawMedia: any) => {
  ...
  return {
    posts: (rawMedia.data as any[]).slice(0, 20).map((post) => ({
      ...
      engRate: post.reach
        ? +(
            ((post.like_count + post.comments_count) / post.reach) *
            100
          ).toFixed(2)
        : null,
