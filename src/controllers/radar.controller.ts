 ...
 */
export const getIntelligence = async (req: FastifyRequest<{ Querystring: RadarQuery }>, reply: FastifyReply) => {
  ...
  try {
    ...
    return success(reply, {
      intelligence,
      meta: { niche, platform, archetype, generatedAt: new Date().toISOString(), fromCache: (intelligence as any).fromCache || false },
    });
  } catch (err) {
 ...
 */
export const getCompetitors = async (req: FastifyRequest<{ Querystring: RadarQuery }>, reply: FastifyReply) => {
  const user = req.user as any;
  try {
    const ctx = getPlatformContext(user);
    const niche     = req.query.niche || ctx.niche;
 ...
 */
export const getInspiration = async (req: FastifyRequest<{ Querystring: RadarQuery }>, reply: FastifyReply) => {
  const user = req.user as any;
  try {
    const ctx = getPlatformContext(user);
    const niche         = req.query.niche || ctx.niche;
