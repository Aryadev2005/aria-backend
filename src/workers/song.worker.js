...
const processSongJob = async (job) => {
  ...
  try {
    ...
    if (!songs || songs.length < 10) {
      ...
      if (songs) {
        allSongs = allSongs.concat(
          songs.map(s => ({ ...s, source: 'jiosaavn', streams_today: 0 }))
        )
      }
    }

    if (allSongs.length === 0) {
      allSongs = FALLBACK_SONGS.map(s => ({ ...s, source: 'fallback', streams_today: 0 }))
...
// Scheduling is handled by setInterval in queue.js — nothing to instantiate here.
...
if (require.main === module) {
  ;(async () => {
    try {
      ...
      await connectRedis()
      await connectDB()
      await processSongJob({ id: 'standalone', data: {} })
      logger.info('Standalone song job complete')
    } catch (err) {
      logger.error({ err }, 'Standalone song job failed')
      process.exit(1)
    }
  })()
}
